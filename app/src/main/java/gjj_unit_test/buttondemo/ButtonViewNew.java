 ...
 * 邮箱：Gujj512@163.com
 */
public class ButtonViewNew extends LinearLayout {
    ...
    public ButtonViewNew(Context context) {
        super(context);
        this.context = context;
        init();
    }

    public ButtonViewNew(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        ...
        screenWidth = ScreenUtils.getScreenWidth(context)-dipToPx(view_padding);
//        viewWidth = screenWidth / btnNumber;
        viewWidth = (int)(screenWidth / (btnNumber));
        textSize=(int)( 20*height_B);
        init();
    }

    public void init() {
        tips=new String[]{"偏低","正常","正常高值","轻度","中度","重度"};
        numbers=new double[]{18.5,24.0,28.0,35.0,50};
...
//        this.setGravity(Gravity.CENTER);
    ...
    public void initButton(int position) {
        ...
        layoutParams.setMargins(dipToPx(viewWidth/2)-padding*2,topPadding,0,0);
        ...
        for (int i = 0; i < colors.length; i++) {
            ...
            if(position==i+1){
                length+=(int)( viewWidth*bili+padding);
                if(i<numbers.length){
                    addView(linearLayout2,i,length);
                }
...
//            textView.setPadding(4,0,4,0);
    ...
    public void addView(LinearLayout linearLayout2,int position,int length){
